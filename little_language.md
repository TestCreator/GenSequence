...
Globals are declared first, and of form:

`<Global>::= <Symbol Name> <Value>`
...
A Range object is required for every Parm object. Moreover, if that Parm can be generated by a cardioid or _cardioid distribution, you should specify the necessary Range objects for favorites and outliers.

A Range object is specified by:

```
<S>        ::= <varname> Range <opener> <argset> <closer>
<varname>  ::= "Total"<alphanum> | <alphanum>
<alphanum> ::= any alphanumeric variable name
<opener>   ::= "(" | "["
<closer>   ::= ")" | "]"
<argset>   ::= <low> "," <high> <opt-args> 
<low>      ::= "low :" <dp>
<high>     ::= "high :" <dp>
<opt-args> ::= "," <ave> <dev> <lp> <rp> | ""
<ave>      ::= "ave:" <dp> "," | ""
<dev>      ::= "dev:" <dp> "," | ""
<lp>       ::= "lp:" <dp> "," | ""
<rp>       ::= "rp:" <dp> | ""
<dp>       ::= <int> | <float> // => can be any int or float number
```

The name of the Range object used for the construction of a Parm should be prepended with "Total". So if you name your Range object `TotalTemps`, it will be used in the construction of the `Temps` Parm. If the Range object will be used for cardioid distribution it can be named anything you want but should not start with "Total".

\<low> and \<high> are required for a Range declaration, but \<ave>, \<dev>, \<lp>, and \<rp> are all optional (though specifying them allows for greater control over the values of hte . If specified, they must be comma-separated. The bounds of inclusion are the same as in mathematical notation. "[" and "]" are inclusive of the low and high values, and "(" and ")" are exclusive.

Here are a few example derivations of the grammar that you should write in the .prm file, and their resultant instantiation in the gen.py code:

```
//* Write this in the .prm file... *//
TotalLats Range(low: 38.58, high: 51.23, ave: 45.84, dev: 1.8, lp: 43.3, rp: 48.8]
North Range[low: -128.6085, high: -121.0184]

//* Preprocessor comes in here and transforms... *//
//* To get these declarations! *//
TotalLats = Range(38.58, 51.23, exclusive_lower=True, exclusive_upper=False, ave=44.9, dev=1.8, lp=43.3, rp=48.8)
North = Range(-128.6085, -121.0184, exclusive_lower=False, exclusive_upper=False)
```
...
## Range Objects
Ranges are helpful objects that merely describe a range of possible values. It is similar to python's built-in range, but this has added functionality of ranging floats, exclusive bounds, and selection of a random point within. the selection type is limited to uniform distribution generation.
...
@Vertical is all the columns: Parameter Objects and their names. The following sections will describe Vertical in detail.
...
The words "low", "high", "ave", "dev", and "peak" are keywords for the arguments to these functions, but they can be any object type. For example:
```
int low 0
int high 5
```
